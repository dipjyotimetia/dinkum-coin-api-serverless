...
pipeline {
	...
	stages {
		...
		stage("Performance Test") {
			...
			script {
				docker.image('denvazh/gatling:2.2.2').withRun("--rm -i -v ${env.WORKSPACE}/test/DinkumCoin.Api.PerformanceTests/user-files:/opt/gatling/user-files -v ${env.WORKSPACE}/test/DinkumCoin.Api.PerformanceTests/results:/opt/gatling/results   denvazh/gatling  -s DinkumCoinSimulation") { c -> 
